      ...
      <div className="dashboard-layout" style={{ display: 'flex', gap: '48px', padding: '32px 48px 40px 48px', flex: '1' }}>
        ...
        <div className="sidebar" style={{ width: '260px', flexShrink: '0', background: '#FFFFFF', borderRadius: '28px', padding: '24px 0', border: '1px solid #E9EDF2', height: 'fit-content' }}>
          ...
          <div className="sidebar-nav" style={{ display: 'flex', flexDirection: 'column' }}>
            <a href="#" onClick={(e) => { e.preventDefault(); navigate('/voter/dashboard'); }} className="sidebar-link active" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 20px', textDecoration: 'none', fontSize: '14px', fontWeight: '500', color: '#002F6C', background: '#F0F7FF', borderLeft: '3px solid #002F6C' }}>Profile</a>
        ...
        <div className="main-content" style={{ flex: '1', display: 'flex', flexDirection: 'column', gap: '28px' }}>
          {/* Welcome Card */}
          <div className="welcome-card" style={{ background: 'linear-gradient(105deg, #0B2F5C 0%, #16456E 100%)', borderRadius: '24px', padding: '28px 32px', color: '#FFFFFF' }}>
            <h3 style={{ fontSize: '14px', fontWeight: '500', letterSpacing: '0.5px', opacity: '0.8', margin: '0 0 8px 0', color: '#FFFFFF' }}>Welcome back,</h3>
            <h1 style={{ fontSize: '28px', fontWeight: '700', margin: '0 0 12px 0', color: '#FFFFFF' }}>{voter?.name || 'Student'}</h1>
            <div className="status-badge" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(255, 255, 255, 0.12)', padding: '6px 14px', borderRadius: '40px', fontSize: '13px', fontWeight: '500', color: '#FFFFFF' }}>
              <span className="dot" style={{ width: '8px', height: '8px', background: '#2DCC70', borderRadius: '50%' }}></span>
              <span>Active Elections</span>
            </div>
          </div>

          {/* STATUS + CAMPUS + CATEGORY row */}
          <div className="info-strip" style={{ display: 'flex', gap: '40px', background: 'white', borderRadius: '20px', padding: '20px 28px', border: '1px solid #E9EDF2', flexWrap: 'wrap' }}>
          ...
          <div className="two-col-grid" style={{ display: 'flex', gap: '28px' }}>
            ...
            <div className="card" style={{ background: 'white', borderRadius: '24px', border: '1px solid #E9EDF2', flex: '1', overflow: 'hidden' }}>
              <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 24px', background: 'linear-gradient(90deg, #0B2F5C 0%, #16456E 100%)', color: '#FFFFFF' }}>
                <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#FFFFFF', margin: 0 }}>Active Elections</h3>
                <a href="#" onClick={(e) => { e.preventDefault(); navigate('/voter/ballot'); }} className="view-link" style={{ fontSize: '13px', color: 'rgba(255,255,255,0.9)', textDecoration: 'none', fontWeight: '500' }}>Vote Now →</a>
              </div>
              <div className="card-content" style={{ padding: '8px 0', minHeight: '120px', position: 'relative' }}>
                {loading && <div style={{ padding: '24px', textAlign: 'center', fontSize: '13px', color: 'black' }}>Loading active sessions...</div>}
                {!loading && activeElections.length === 0 ? (
                  <div style={{ padding: '24px', textAlign: 'center', fontSize: '13px', color: 'black' }}>No active elections at the moment.</div>
                ) : (
                  !loading && activeElections.map((election) => (
                    <div key={election.id} className="election-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px', borderBottom: '1px solid #F0F4FA' }}>
                      <div>
                        <div className="election-title" style={{ fontWeight: '700', fontSize: '15px', color: '#1F2A44', marginBottom: '4px' }}>{election.title}</div>
                        <div className="election-deadline" style={{ fontSize: '12px', color: '#2DCC70', fontWeight: '500' }}>Active Now</div>
                      </div>
                      <div className="time-badge" style={{ background: '#E8F5E9', padding: '6px 14px', borderRadius: '30px', fontSize: '12px', fontWeight: '600', color: '#2E7D32' }}>VOTE</div>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="card" style={{ background: 'white', borderRadius: '24px', border: '1px solid #E9EDF2', flex: '1', overflow: 'hidden' }}>
              <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 24px', background: 'linear-gradient(90deg, #0B2F5C 0%, #16456E 100%)', color: '#FFFFFF' }}>
                <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#FFFFFF', margin: 0 }}>Security Log</h3>
              </div>
              <div className="card-content" style={{ padding: '8px 0', maxHeight: '300px', overflowY: 'auto' }}>
                {!loading && securityLogs.length === 0 ? (
                  <div style={{ padding: '24px', textAlign: 'center', fontSize: '13px', color: 'black' }}>No recent activity logged.</div>
                ) : (
                  securityLogs.map((log, idx) => (
                    <div key={idx} className="log-row" style={{ padding: '16px 24px', borderBottom: idx < securityLogs.length - 1 ? '1px solid #F0F4FA' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div>
                        <div className="log-title" style={{ fontWeight: '600', fontSize: '14px', color: '#1F2A44', marginBottom: '4px' }}>{log.action}</div>
                        <div className="log-meta" style={{ fontSize: '12px', color: '#7E8C9E' }}>IP: {log.ip_address || '---'}</div>
                      </div>
                      <div className="log-time" style={{ fontSize: '11px', color: '#7E8C9E' }}>{new Date(log.timestamp).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</div>
                    </div>
                  ))
                )}
              </div>
